...
const fetchEntityCall = id => fetchSubgraph({
  query: `{
    call(id: "${id}") {
      id
      value
  ...
  new Promise((resolve, reject) => {
    ...
    const checkSubgraphFailedWithPoI = async () => {
      try {
        ...
        // Get Call that the mappings tried to save before the DeterministicError happened.
        let callResult = await fetchEntityCall("something")

        let call = callResult.data && callResult.data.call
